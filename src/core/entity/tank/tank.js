...
class Tank extends Entity {
  ...
  drive(power) {
    ...
    let dx =
      power *
      Math.sin(this.direction.getRadians()) *
      Tank.driveSpeed *
      Game.delta;
    let dy =
      -1 *
      power *
      Math.cos(this.direction.getRadians()) *
      Tank.driveSpeed *
      Game.delta;
  ...
  shoot() {
    if (this.remainingTimeToReload <= 0) {
      Spawner.bullet(
        this.position,
        this.direction.getRadians() + this.turretDirection.getRadians(),
        this.id
      );
  ...
  rotateTurret(power) {
    power = Math.clamp(power, -1, 1);
    this.turretDirection.addDegrees(
      power * Tank.turretRotationSpeed * Game.delta
    );
  }
  ...
  drawTracks(ctx) {
    ctx.beginPath();
    ctx.rect(
      -Tank.trackSpacing / 2 - Tank.trackWidth,
      -(Tank.trackLength / 2),
      Tank.trackWidth,
      Tank.trackLength
    );
  ...
  drawTurret(ctx) {
    ...
    ctx.beginPath();
    ctx.rect(
      -Tank.turretBaseWidth / 2,
      -Tank.turretBaseWidth / 2,
      Tank.turretBaseWidth,
      Tank.turretBaseWidth
    );
